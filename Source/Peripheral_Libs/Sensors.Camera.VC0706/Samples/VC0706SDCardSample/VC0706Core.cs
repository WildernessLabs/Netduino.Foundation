...
namespace Camera_VC0706
{
    public class VC0706Core : IDisposable
    {
        ...
        public void Initialize(SerialPort comPort)
        {
            _comPort = comPort;
        ...
        private int GetTimeoutMilliSec(int bufferLength) {
            var timeout = ((bufferLength / (_comPort.BaudRate >> 3)) * 1000) + CameraDelayMilliSec + TimeoutToleranceMilliSec;
            return timeout;
        }
        ...
        {
            RunCommand(command, args, expectedResponseLength, _comPort, flushComPort);
        }

        protected void RunCommand(Command command, byte[] args, int expectedResponseLength, SerialPort comPort, bool flushComPort = true)
        {
            if (flushComPort)
