...
#else
using PortList = System.Collections.Generic.List<Netduino.Foundation.Port>;
#endif
...
namespace Netduino.Foundation
{
	public class Port
	{
        ...
        public void ConnectTo (Netduino.Foundation.Messaging.ControlServer server, bool writeable = false, string name = null)
        {
            var v = server.RegisterVariable (
                name ?? FullNameWithUnits,
                Value,
                writeable ? (Netduino.Foundation.Messaging.VariableChangedAction)(x => Value = x.DoubleValue) : null);
