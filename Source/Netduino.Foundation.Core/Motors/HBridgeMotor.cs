...
namespace Netduino.Foundation.Motors
{
    public class HBridgeMotor : Block, IDCMotor
    {
        ...
        {
            var leftPwm = new Netduino.Foundation.Devices.PwmOutputPin (enable);
            var leftMotor = new HBridgeMotor (leftPwm);
            leftMotor.A1Output.ConnectTo (new Netduino.Foundation.Devices.DigitalOutputPin (a1).Input);
            leftMotor.A2Output.ConnectTo (new Netduino.Foundation.Devices.DigitalOutputPin (a2).Input);
            return leftMotor;
        }
    }
}
